...
typedef char** (*FUNC_XLDESK_CORE_MAIN)(int*);
typedef void (*FUNC_RUSTDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_XLDESK_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libxldesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libxldesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_XLDESK_CORE_MAIN rustdesk_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "rustdesk_core_main_args");
  if (!rustdesk_core_main)
  {
    std::cout << "Failed to get xldesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (!c_args)
  {
    ...
    // std::cout << "XLDESK [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"XLDESK";
  FUNC_XLDESK_GET_APP_NAME get_xldesk_app_name = (FUNC_XLDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_xldesk_app_name");
  if (get_xldesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_xldesk_app_name(app_name_buffer, 512) == 0) {
