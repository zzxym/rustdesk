...
const UPDATE_TEMP_DIR: &str = "/tmp/.xldeskupdate";
...
// remove just one app from all the permissions: tccutil reset All com.carriez.xldesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.carriez.xldesk", &bundleid);
    }
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/xldesk.app/ --args --cm`,","},{"old_str":
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // xldesk.app/Contents/MacOS/xldesk
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
    ...
    if is_installed_daemon && !is_service_stopped {
        ...
        let update_body = format!(
            r#"
do shell script "
pgrep -x 'xldesk' | grep -v {} | xargs kill -9 && rm -rf /Applications/xldesk.app && ditto '{}' /Applications/xldesk.app && chown -R {}:staff /Applications/xldesk.app && xattr -r -d com.apple.quarantine /Applications/xldesk.app
" with prompt "xldesk wants to update itself" with administrator privileges
    "#,
            std::process::id(),
            app_dir,
            get_active_username()
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/xldeskUpdate";
...
fn update_extracted(target_dir: &str) -> ResultType<()> {
    let exe_path = format!("{}/xldesk.app/Contents/MacOS/xldesk", target_dir);
    let _child = unsafe {
